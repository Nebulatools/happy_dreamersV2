...
export const PUT = withApi(
  async ({ params, requestId }) => {
    ...
    const { superseded } = await planRepo.supersedePlan(toObjectId(childId), toObjectId(planId))
    return stdOk({ superseded }, requestId)
  },
  { auth: 'user', validate: { params: paramsSchema }, rateLimit: { limit: 30, windowMs: 60_000, key: 'v2_child_plan_supersede' } }
)
