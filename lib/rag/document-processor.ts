...
export async function processPdfFile(filePath: string, fileName: string): Promise<Document[]> {
  try {
    ...
    console.log(`🧠 Usando Gemini 2.0 Flash para extraer texto de: ${fileName}`);
    ...
    console.log(`✅ PDF procesado con Gemini: ${fileName} - ${text.length} caracteres, ${documents.length} chunks`);
    return documents;
    
  } catch (error) {
    console.error(`❌ Error procesando PDF con Gemini ${fileName}:`, error);
