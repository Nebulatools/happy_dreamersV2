// Genera eventos de JULIO 2025 para Luna García
// Muestra mejoras después de implementar Plan 0 (consolidación de siestas)
...
async function main() {
  ...
  console.log('🌟 GENERACIÓN DE EVENTOS - JULIO 2025 (Luna García)\n')
  console.log('📅 Implementando mejoras del Plan 0:')
  ...
  try {
    ...
    console.log('📊 GENERANDO EVENTOS DE JULIO 2025 (31 DÍAS)')
    console.log('=' .repeat(60))

    // Julio tiene progresión gradual: primeras 2 semanas con 2 siestas, luego 1 siesta
    for (let day = 1; day <= 31; day++) {
      const isTransitioning = day <= 14  // Primeras 2 semanas aún en transición

      // SUEÑO NOCTURNO - Más consistente en julio
      // Plan 0 sugiere: bedtime 20:00, wake 6:57
      const sleepStart = dtLocal(2025, 7, day, 20, 0)
      sleepStart.setMinutes(sleepStart.getMinutes() + randInt(-15, 20))  // Menos variación

      const sleepEnd = dtLocal(2025, 7, day + 1, 6, 57)
      ...
      if (randInt(1, 100) <= 15) {
        const nwStart = dtLocal(2025, 7, day + 1, choice([2, 3]), randInt(0, 59))
      ...
      if (isTransitioning) {
        ...
        const nap2Start = dtLocal(2025, 7, day, 14, 0)
        ...
        const napStart = dtLocal(2025, 7, day, 12, 15)  // Según Plan 0
    ...
    // CALCULAR ESTADÍSTICAS DE JULIO
    console.log('\n' + '=' .repeat(60))
    console.log('📈 ESTADÍSTICAS DE JULIO 2025')
    console.log('=' .repeat(60))

    // Sueño nocturno
    const sleeps = events.filter(e => e.eventType === 'sleep')
    const sleepDurations = sleeps.map(e => e.duration)
    const avgSleepJuly = Math.round(sleepDurations.reduce((a, b) => a + b, 0) / sleepDurations.length)

    const wakeTimes = sleeps.map(e => new Date(e.endTime))
    const wakeMinutes = wakeTimes.map(d => d.getHours() * 60 + d.getMinutes())
    const avgWakeJuly = Math.round(wakeMinutes.reduce((a, b) => a + b, 0) / wakeMinutes.length)
    ...
    const avgBedtimeJuly = Math.round(bedtimeMinutes.reduce((a, b) => a + b, 0) / bedtimeMinutes.length) % (24 * 60)
    ...
    console.log('\n🌙 SUEÑO NOCTURNO:')
    console.log(`   Duración promedio: ${avgSleepJuly} min (${humanDuration(avgSleepJuly)})`)
    console.log(`   Hora despertar: ${formatTime(avgWakeJuly)}`)
    console.log(`   Hora acostar: ${formatTime(avgBedtimeJuly)}`)

    console.log('\n💤 SIESTAS:')
    console.log(`   Total siestas: ${naps.length}`)
    console.log(`   Promedio por día: ${(naps.length / 31).toFixed(1)}`)
    ...
    console.log(`   Promedio por día: ${(feedings.length / 31).toFixed(1)}`)

    console.log('\n😴 DESPERTARES NOCTURNOS:')
    const nightWakings = events.filter(e => e.eventType === 'night_waking')
    console.log(`   Total: ${nightWakings.length}`)
    console.log(`   Promedio por noche: ${(nightWakings.length / 31).toFixed(1)}`)
    ...
    console.log('\n' + '=' .repeat(60))
    console.log('📊 COMPARACIÓN JUNIO vs JULIO 2025')
    console.log('=' .repeat(60))
    ...
    console.log('\n🌙 SUEÑO NOCTURNO:')
    console.log(`   Duración:    Junio ${juneStats.avgSleep} min → Julio ${avgSleepJuly} min (${avgSleepJuly > juneStats.avgSleep ? '+' : ''}${avgSleepJuly - juneStats.avgSleep} min)`)
    console.log(`   Despertar:   Junio ${formatTime(juneStats.avgWake)} → Julio ${formatTime(avgWakeJuly)}`)
    console.log(`   Acostar:     Junio ${formatTime(juneStats.avgBedtime)} → Julio ${formatTime(avgBedtimeJuly)}`)

    console.log('\n💤 SIESTAS:')
    console.log(`   Cantidad:    Junio ${juneStats.totalNaps} → Julio ${naps.length} (${naps.length < juneStats.totalNaps ? '⬇️' : '⬆️'} ${Math.abs(naps.length - juneStats.totalNaps)})`)
    console.log(`   Por día:     Junio 2.0 → Julio ${(naps.length / 31).toFixed(1)} (⬇️ Consolidación exitosa)`)
    console.log(`   Duración:    Junio ${juneStats.avgNapDuration} min → Julio ${avgNapDuration} min (+${avgNapDuration - juneStats.avgNapDuration} min)`)

    console.log('\n😴 DESPERTARES:')
    console.log(`   Nocturnos:   Junio ${juneStats.nightWakings} → Julio ${nightWakings.length} (${nightWakings.length < juneStats.nightWakings ? '⬇️ Mejora' : '⬆️'} ${Math.abs(nightWakings.length - juneStats.nightWakings)})`)

    console.log('\n🍼 TOMAS NOCTURNAS:')
    console.log(`   Total:       Junio ${juneStats.nightFeedings} → Julio ${nightFeedings.length} (${nightFeedings.length === 0 ? '✅ ELIMINADAS' : nightFeedings.length})`)

    // Mejoras identificadas
    console.log('\n' + '=' .repeat(60))
    console.log('✨ MEJORAS IDENTIFICADAS (Plan 0 → Julio)')
    console.log('=' .repeat(60))

    const improvements = []

    if (avgSleepJuly > juneStats.avgSleep) {
      improvements.push(`✅ Sueño nocturno +${avgSleepJuly - juneStats.avgSleep} min más largo`)
    }

    if (naps.length < juneStats.totalNaps) {
      improvements.push(`✅ Consolidación de siestas: ${juneStats.totalNaps}→${naps.length} (${(naps.length / 31).toFixed(1)} por día)`)
    }
    ...
    if (nightFeedings.length === 0 && juneStats.nightFeedings > 0) {
      improvements.push(`✅ Tomas nocturnas eliminadas (0 en julio vs ${juneStats.nightFeedings} en junio)`)
    }
    ...
    console.log('=' .repeat(60))
    console.log(`\n📅 Eventos de julio 2025: ${events.length} eventos creados`)
    console.log(`👶 Child ID: ${LUNA_CHILD_ID}`)
    console.log('\n🎯 Siguiente paso: Generar Plan 1 con:')
    console.log(`   node scripts/02_generate-plan0-july-1-2025.js ${LUNA_CHILD_ID} ${PARENT_ID}`)
    console.log('\n   O usa el API para generar Plan 1 automáticamente')
